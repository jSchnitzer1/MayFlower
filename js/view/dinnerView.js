...
var DinnerView = function (container, model) {
    ...
    var select_dish = container.find(".select_dish");
    var select_dish_label = container.find(".select_dish_label");
    var search_txt = container.find("#search_txt");
    ...
    var all = mainCourse.concat(sideDish).concat(dessert);



    plusButton.on('click', function () {
        var value = parseInt(numberOfGuests.val());
    ...
    select_dish.focus(function () {
        select_dish_label.toggleClass('changed');
    });

    select_dish.focusout(function () {
        select_dish_label.toggleClass('changed');
    });

    select_dish.change(function () {
        var $selected = container.find(".select_dish option:selected");
        $selected.each(function () {
            switch(parseInt($(this).val())){
                case 0:
                    search_txt.autocomplete('option', 'source', all);
                    break;
                case 1:
                    search_txt.autocomplete('option', 'source', mainCourse );
                    break;
                case 2:
                    search_txt.autocomplete('option', 'source', sideDish);
                    break;
                case 3:
                    search_txt.autocomplete('option', 'source', dessert);
                    break;
